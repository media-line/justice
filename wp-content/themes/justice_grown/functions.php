...
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Sibebar', // Название в админке
...
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Modal', // Название в админке
	'id' => "modal", // идентификатор для вызова в шаблонах
	'description' => 'Виджет модального окна', // Описалово в админке
	'before_widget' => '<div id="%1$s" class="widget %2$s">', // разметка до вывода каждого виджета
	'after_widget' => "</div>\n", // разметка после вывода каждого виджета
	'before_title' => '<span class="widgettitle">', //  разметка до вывода заголовка виджета
	'after_title' => "</span>\n", //  разметка после вывода заголовка виджета
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
		...
		wp_enqueue_style( 'main', get_template_directory_uri().'/style.css' ); // начальные стили шаблона
		wp_enqueue_style( 'template', get_template_directory_uri().'/css/template.css' ); // основные стили шаблона
		wp_enqueue_style( 'custom', get_template_directory_uri().'/css/custom.css' ); // пользовательские стили шаблона
	}
}
